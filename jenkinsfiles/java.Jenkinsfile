...
node {
	...
	sh "docker run gesellix/trufflehog --json --rules https://github.com/dxa4481/truffleHogRegexes/blob/master/truffleHogRegexes/regexes.json ${props['deploy.gitURL']} > trufflehog"
	sh "cat trufflehog"
	}
	catch (error) {
				currentBuild.result='FAILURE'
